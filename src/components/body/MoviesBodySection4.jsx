...
import { Box, Card, CardBody, CardFooter, CardHeader, Text, Image, Flex, Button, Divider, Skeleton } from "@chakra-ui/react";
...
export default function MoviesBodySection4()
{
    ...
    useEffect(() => {
        axios.get("https://api.themoviedb.org/3/tv/top_rated?api_key=" + apiKey).then((response) => {
    ...
    return (
        ...
        <Flex justifyContent={"space-between"} alignItems={"center"}>
            <Text color={"white"} fontSize={"lg"} fontWeight={"semibold"}>Top Rated TV Series</Text>
        ...
        >
            ...
            <SwiperSlide className="">
                <Card bg={"whiteAlpha.100"} mt={7} borderRadius={"15px"} overflow={"hidden"} border={"none"} outline={"none"} className="">
                    ...
                    <CardBody>
                        <Link className="text-white hover:underline" to={`/details/tv/${data.id}`} display={"inline-block"} variant={"link"} colorScheme={"yellow"} fontSize={"md"} color={"white"} fontWeight={"medium"}>
                            { data.name }
                        </Link>
